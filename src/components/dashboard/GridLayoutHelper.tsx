...
export function GridLayoutHelper() {
  ...
  const snapToGrid = () => {
    ...
    widgets.forEach((widget) => {
      ...
      const gridX =
        Math.round(widget?.position?.x ?? 1 / cellWidth) * cellWidth;
      const gridY =
        Math.round(widget?.position?.y ?? 1 / cellHeight) * cellHeight;

      // Calculate size to be a multiple of grid cells
      const gridWidth =
        Math.max(1, Math.round(widget.size.width / cellWidth)) * cellWidth;
      const gridHeight =
        Math.max(1, Math.round(widget.size.height / cellHeight)) * cellHeight;
